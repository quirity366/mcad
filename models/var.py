...
# import clip
from models.LongCLIP.model import longclip  # 改成longclip
import os
import config
...
class ClipEmbed(nn.Module):
    def __init__(self, class_num, output_dim, device, dtype=torch.float32):
        super().__init__()
        ...
        #  LongCLIP 
        model_path = config.CLIP_CKPT
        self.clip_model, _ = longclip.load(model_path, device=self.device)
        ...
        self.idx_to_text = classes
        # assert self.class_num == len(classes)
        if (self.class_num != len(classes)):
            raise ValueError(f"Mismatch between class_num ({self.class_num}) and number of classes found ({len(classes)}).")    

        self.fc = nn.Linear(1024, output_dim).to(self.device).to(self.dtype)

    def forward(self, idx):
        idx = idx.to(self.device)

        batch_size = idx.size(0)
        combined_features = torch.zeros((batch_size, 1024), device=self.device, dtype=self.dtype)

        for i in range(batch_size):
            if idx[i] >= self.class_num:
                word1, word2 = '', ''
            else:
                class_text = self.idx_to_text[idx[i]]
                word1, word2 = class_text.split('-', 1)

            with torch.no_grad():
                # tokenize, text_encode
                word1_tokens = longclip.tokenize(word1).to(self.device)
                word2_tokens = longclip.tokenize(word2).to(self.device)
                word1_features = self.clip_model.encode_text(word1_tokens).to(self.device)
                word2_features = self.clip_model.encode_text(word2_tokens).to(self.device)

            combined_features[i] = torch.concat((word1_features, word2_features), dim=-1).to(self.dtype)
...
class VAR(nn.Module):
    def __init__(
        ...
        self.class_emb = ClipEmbed(self.num_classes, self.C, device=dist.get_device())
        nn.init.trunc_normal_(self.class_emb.fc.weight.data, mean=0, std=init_std)
