...
class CAR(VAR):
    ...
    def car_inference(
        ...
        blend=False, good_B3HW=None, mask_B3HW=None
    ) -> torch.Tensor:
        if g_seed is None:
            rng = None
        else:
        ...
        print(label_B, torch.cat((label_B, torch.full_like(label_B, fill_value=self.num_classes))).shape)
        ...
        print(sos.shape, lvl_pos.shape, B, self.first_l, self.pos_start.shape)
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            if si != self.num_stages_minus_1:  # prepare for next stage
                ...
                #     blended_next_token_map = next_token_map \
                #                     * mask_gt_BL[:, cur_L:cur_L+pn_next**2].unsqueeze(-1) \
                #                     + (torch.ones_like(mask_gt_BL[:, cur_L:cur_L+pn_next**2].unsqueeze(-1)) - mask_gt_BL[:, cur_L:cur_L+pn_next**2].unsqueeze(-1)) \
                #                     * normal_gt_BLC[:, cur_L:cur_L+pn_next**2, :]
                #     next_token_map = blended_next_token_map.to(next_token_map.device)
    ...
    def GetMask4fhatInfer(self, mask_B3HW: torch.Tensor):
        ...
        array = interpolated_tensor[0].permute(1, 2, 0).cpu().numpy()
