...
# https://arxiv.org/pdf/2310.10563
# github：https://github.com/Aiolus-X/RefConv
...
if __name__ == '__main__':
    ...
    refconv = RefConv(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=None, groups=1, map_k=3).to(device)  # 将模型移动到设备
    input = torch.rand(1, 32, 256, 256).to(device)  # 将输入数据移动到设备

    output = refconv(input)
