...
class VectorQuantizer2(nn.Module):
    ...
    def mask_to_multiscale_efficient(self, msk_B3HW: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> torch.Tensor:
        B, C, H, W = msk_B3HW.shape
        
        if C == 3:
            msk_B1HW = msk_B3HW.mean(dim=1, keepdim=True)
        else:
            msk_B1HW = msk_B3HW
        ...
        for ph, pw in patch_hws:
            if (ph, pw) != (H, W):
                msk_down = F.interpolate(msk_B1HW, size=(ph, pw), mode='area')
            else:
                msk_down = msk_B1HW
            
            msk_flat = msk_down.view(B, -1)  #  (B, ph×pw)
            multiscale_msk_list.append(msk_flat)
        
        multiscale_msk_BL = torch.cat(multiscale_msk_list, dim=1)
        
        return multiscale_msk_BL
